...
/*
笔记:
  1. 一次取出 10 个值，这样遍历次数会减少到 1/10
  2. 不够 10 个时，一次取出 1 个
*/
type LengthOfString<S extends string, A extends any[] = []> =
  S extends `${infer S0}${infer S1}${infer S2}${infer S3}${infer S4}${infer S5}${infer S6}${infer S7}${infer S8}${infer S9}${infer R}`
    ? LengthOfString<R, [S0, S1, S2, S3, S4, S5, S6, S7, S8, S9, ...A]>
