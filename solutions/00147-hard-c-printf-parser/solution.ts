...
笔记:
  1. 模板字符串的使用，感觉算不上难题
*/
type ParsePrintFormat<S extends string, T extends any[] = []> = S extends `${any}%${infer R}`
  ? R extends `${infer F}${infer RS}`
    ? F extends keyof ControlsMap
      ? ParsePrintFormat<RS, [...T, ControlsMap[F]]>
      : ParsePrintFormat<RS, T>
    : T
  : T
