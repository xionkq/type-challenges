...
/*
笔记:
  1. [T] extends [never] 并不是为了使用数组判断是否继承，而是通过中括号语法避免条件类型分配的默认行为
  2. 通过使用 number extends T['length'] 可以区分元组，因为元组的长度都是文字类型的数字，而 1 extends number，但反过来不行
*/
...
type cases = [
  ...
  Expect<Equal<IsTuple<[number, number]>, true>>,
